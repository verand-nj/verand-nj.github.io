/* ============================================================
   ...
   ============================================================ */
const CONFIG = {
  herName:  "Vivi",            // son prénom (laisse "" si tu veux la surprise)
  myName:   "Verand",          // ton prénom
  myPhone:  "33695492708",     // ton numéro international SANS le "+"  (FR: 06 12 34 56 78 -> "33612345678")
  web3formsKey: "627e14a2-d95a-443a-a27f-077ff2707de2", // ta clé Web3Forms
};
/* ============================================================ */
...
/* ---- Écran 1 : ouverture de l'enveloppe ----
   (Le titre est écrit en dur dans le HTML : on ne le touche plus ici) */
...
const taunts=["Non", "whèè maaa faut réfléchir un non ?", "T'es sûre ?", "Réfléchis…", "Vraiment ?", "Attends…", "Regarde d'abord le Oui 👀", "Dernière chance", "Allez quoi 🥺", "Impossible 😌"];
let dodges=0;
function fleeNo(){
  dodges++;
...
/* ---- Écran 4 : activité ----
   Pour modifier : change/ajoute des lignes ici (e=emoji, t=titre, s=sous-titre) */
const acts=[
  {e:"🍝",t:"Restau",s:"J'espère que tu as un grand appétit"},
  {e:"🎬",t:"Ciné",s:"Mouais non, t'aimes pas"},
  {e:"🍹",t:"Un verre",s:"Bar cosy"},
  {e:"🚶",t:"Balade",s:"Petite balade tu connais"},
  {e:"☕",t:"Café",s:"Simplement simple quoi…"},
  {e:"🎲",t:"Surprise",s:"Je gère tout, à moi de te surprendre 😏"}
];
...
acts.forEach(it=>{
  ...
  b.className='choice';
  b.innerHTML=`<span class="emoji">${it.e}</span><span class="c-title">${it.t}</span><small>${it.s}</small>`;
  b.onclick=()=>{
...
/* ---- Écran 5 : récap + mot doux + messages de secours ---- */
function frDate(iso){
...
function buildRecap(){
  ...
  // Le petit mot doux (réécris-le comme tu veux) :
  ...
  // Le message pré-rempli (boutons de secours) :
...
async function sendEmail(dTxt, tTxt){
  ...
  try{
    ...
    else{ status.innerHTML="⚠️ L'envoi auto n'a pas marché — utilise un bouton ci-dessous."; }
  }catch(_){
